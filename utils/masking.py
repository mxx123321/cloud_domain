...
def flip_values(matrix, image,flip_prob):
    # 生成与输入矩阵相同形状的随机数矩阵
    random_matrix = torch.rand(matrix.size(), device=image.device)

    # 将原本为1的值以flip_prob的概率变为0
    matrix_step1 = torch.where(matrix == 1, torch.where(random_matrix < flip_prob, 0, matrix), matrix)

    # 将原本为0的值全部变为1
    matrix_setp2 = torch.where(matrix == 0, 1, matrix_step1)

    return matrix_setp2*image
...
class Masking(nn.Module):
    ...
    def forward(self, img: Tensor):
        ...
        if self.ratio != 0: #means only augmentation
