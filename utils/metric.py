...
def cal_average(tensor1, tensor_target):
    a1 = tensor_target[:]- tensor1[:]
    sum = torch.sum(pow(a1, 2), 1)
    distence = pow(sum, 0.5)
    sum = torch.sum(distence)

    return (sum / tensor1.shape[0])